...
phi_GT = groundtruth;
% 3.4 Tính sai số
RMSE(k,1) = compute_rmse(phi_goldstein, phi_GT);
RMSE(k,2) = compute_rmse(phi_tie_dct, phi_GT);
RMSE(k,3) = compute_rmse(phi_quality,   phi_GT);
RMSE(k,4) = compute_rmse(phi_wls,  phi_GT);
RMSE(k,5) = compute_rmse(phi_proposed,   phi_GT);

figure; hold on;

for i = 1:5
    scatter(i*ones(k,1), RMSE(:,i), 25);
end

xticks(1:5);
xticklabels({"Goldstein","TIE-DCT","Quality","WLS","Proposed"});
ylabel("RMSE");
title("RMSE of all samples for each method");
grid on;
